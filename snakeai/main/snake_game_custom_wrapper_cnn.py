...
class SnakeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if info["snake_size"] == self.grid_size: # Snake fills up the entire board. Game over.
            reward = 6 * self.max_growth * 0.1 # Victory reward
            self.done = True
        ...
        if self.done: # Snake bumps into wall or itself. Episode is over.
            ...
            self.score += 10  # 每吃到一个果子增加10分
            if self.score >= 1300:   # 得分大于等于1300时，奖励提高到5倍
                reward = 5 * (info["snake_size"] / self.grid_size)
            elif self.score >= 1200:   # 得分大于等于1200时，奖励提高到4倍
                reward = 4 * (info["snake_size"] / self.grid_size)
            elif self.score >= 900:    # 得分大于等于900时，奖励提高到3倍
                reward = 3 * (info["snake_size"] / self.grid_size)
            elif self.score >= 600:  # 得分大于等于600分时奖励加倍
                reward = 2 * (info["snake_size"] / self.grid_size)  
            else:
